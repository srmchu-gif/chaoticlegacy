...
const EFFECT_PARTIAL_FAILURE_POLICY_BY_KIND = Object.freeze({
  negateMugic: "cancel_card",
  negateMugicOrAbilityTargeting: "cancel_card",
  disableMugicAndActivated: "cancel_card",
});

function runtimeContractForEffect(effect) {
  const kind = String(effect?.kind || "").trim();
  return {
    priorityWindow: "official",
    targetResolution: "sequential",
    targetRepeatPolicy: "allow_unless_another_target",
    requirementsCheckAt: "resolution",
    costAtomicity: "full",
    stackResolution: "lifo",
    simultaneousTriggerOrder: ["location", "battlegear", "creature", "mugic"],
    unknownEffectPolicy: "allow_noop_log",
    partialFailurePolicy: EFFECT_PARTIAL_FAILURE_POLICY_BY_KIND[kind] || "resolve_partial",
  };
}
...
function normalizeConditionSourceText(value) {
  return String(value || "")
    .toLowerCase()
    .replace(/[()]/g, " ")
    .replace(/\s+/g, " ")
...
function extractStatCheckClauses(text) {
  const clauses = [];
  const regex = /Stat Check\s+(Courage|Power|Wisdom|Speed|Mugic\s*counters?)\s+(\d+)\s*:\s*([^.]*)/gi;
  const sourceText = String(text || "");
  let match = regex.exec(sourceText);
  while (match) {
    const stat = parseConditionalStatWord(match[1]);
    const threshold = Number(match[2]);
    const body = String(match[3] || "").trim();
    if (stat && Number.isFinite(threshold) && body) {
      clauses.push({
        type: "stat_check",
        stat,
        threshold,
        comparator: "selfGte",
        body,
        normalizedBody: normalizeConditionSourceText(body),
      });
    }
...
function shouldKeepStatCheckPrefix(bodyText) {
  const normalized = normalizeConditionSourceText(bodyText);
  if (!normalized) {
    return false;
  }
  if (/^deal\s+\d+\s+damage\b/.test(normalized)) {
    return true;
  }
  if (/^choose a battlegear equipped to an opposing creature and destroy it\b/.test(normalized)) {
    return true;
  }
  return false;
}

function attachStatCheckConditionRules(effects, clauses) {
  if (!Array.isArray(effects) || !Array.isArray(clauses) || !clauses.length) {
    return effects;
  }
  const skipKinds = new Set(["conditionalDamage", "destroyBattlegearIfAttackerStatGte"]);
  effects.forEach((effect) => {
    if (!effect || effect.condition_rule || skipKinds.has(String(effect.kind || ""))) {
      return;
    }
    const source = normalizeConditionSourceText(effect.sourceText || "");
    if (!source) {
      return;
    }
    const matched = clauses.find((clause) =>
      clause.normalizedBody === source
      || clause.normalizedBody.includes(source)
      || source.includes(clause.normalizedBody)
    );
    if (!matched) {
      return;
    }
...
function parseAbilityEffects(text) {
  ...
  const statCheckClauses = extractStatCheckClauses(clean);
  ...
  const statCheckPrefixRegex = /Stat Check\s+(Courage|Power|Wisdom|Speed|Mugic\s*counters?)\s+(\d+)\s*:\s*/gi;
  working = working.replace(statCheckPrefixRegex, (fullMatch, _stat, _threshold, offset, sourceText) => {
    const source = String(sourceText || "");
    const start = Number(offset) + String(fullMatch || "").length;
    const remaining = source.slice(start);
    const bodyUntilDot = String(remaining.split(".")[0] || "").trim();
    if (shouldKeepStatCheckPrefix(bodyUntilDot)) {
      return fullMatch;
    }
    return "";
  });
  ...
  const discardThenDrawSingleAttackRegex = /Discard an Attack Card,\s*then draw an Attack Card/gi;
  working = working.replace(discardThenDrawSingleAttackRegex, (_) => {
    effects.push({
      kind: "drawDiscardAttack",
      draw: 1,
      discard: 1,
  ...
  const revealsNewLocationRegex = /reveals?\s+a new active Location/gi;
  working = working.replace(revealsNewLocationRegex, (_) => {
    effects.push({
      kind: "revealNewLocation",
      sourceText: _.trim(),
    });
  ...
  const canMoveAsIfAdjacentRegex = /([A-Za-z'\-\s]+)\s+can move to any space(?:\s+on the board)? as if it were adjacent/gi;
  ...
  const mirageAdjacentMustMoveIntoSpaceRegex =
    /Mirage:\s*If an adjacent Creature can move into this space,\s*it must do so/gi;
  working = working.replace(mirageAdjacentMustMoveIntoSpaceRegex, (_) => {
    effects.push({
      kind: "mustEngageIfPossible",
      sourceText: _.trim(),
    });
  ...
  const iparuJungleRegex =
    /When this becomes the active Location,\s*flip a coin\.[\s\S]*?If tails,\s*shuffle Iparu Jungle into its controller'?s Location Deck and that player reveals a new active Location\.[\s\S]*?Mirage:\s*At the beginning of combat,\s*each engaged Creature in this space gains or loses Energy so its Energy is equal to its Scanned Energy/gi;
  ...
  const returnMandiblorFromDiscardAnyOpenSpaceRegex =
    /Return target Mandiblor Creature Card in your general discard pile to play to any unoccupied space\b/gi;
  working = working.replace(returnMandiblorFromDiscardAnyOpenSpaceRegex, (_) => {
    effects.push({
      kind: "returnCreatureFromDiscardToBoard",
      requiredCreatureTypes: ["mandiblor"],
      target: "self",
  ...
  const strainOfClarityRegex =
    /Choose one:\s*Shuffle target Mirage Location into its controller'?s Location Deck\.[\s\S]*?If the active Location is a Mirage,\s*shuffle it into its controller'?s Location deck and that player reveals a new active Location/gi;
  ...
  const shuffleTargetMirageLocationIntoDeckRegex =
    /Shuffle target Mirage Location into its controller'?s Location Deck/gi;
  working = working.replace(shuffleTargetMirageLocationIntoDeckRegex, (_) => {
    effects.push({
      kind: "realityFieldMirageControl",
      sourceText: _.trim(),
      targetSpec: {
  ...
  const setEngagedEnergyToScannedAtBeginCombatRegex =
    /At the beginning of combat,\s*each engaged Creature in this space gains or loses Energy so its Energy is equal to its Scanned Energy/gi;
  working = working.replace(setEngagedEnergyToScannedAtBeginCombatRegex, (_) => {
    effects.push({
      kind: "setEngagedEnergyToScannedAtBeginCombat",
      timing: "begin_combat",
      sourceText: _.trim(),
  ...
  const equippedCreatureCannotMoveRegex = /Equipped Creature cannot move/gi;
  working = working.replace(equippedCreatureCannotMoveRegex, (_) => {
    effects.push({
      kind: "cannotMove",
      target: "self",
      sourceText: _.trim(),
  ...
  const equippedCreatureNoAbilitiesRegex = /Equipped Creature has no abilities(?:\s+and cannot gain abilities)?/gi;
  working = working.replace(equippedCreatureNoAbilitiesRegex, (_) => {
    effects.push({
      kind: "suppressTargetCreatureAbilities",
      target: "self",
      sourceText: _.trim(),
  ...
  working = working.replace(exileUpToCardsFromGeneralDiscardRegex, (_, countWord) => {
    const maxTargets = parseCountWord(countWord, 1);
    effects.push({
      kind: "exileGeneralDiscardCards",
      target: "all",
      amount: maxTargets,
  ...
  attachStatCheckConditionRules(effects, statCheckClauses);
  ...
  return effects.map((effect) => {
    ...
    output.runtimeContract = runtimeContractForEffect(output);
    return output;
  });
}
