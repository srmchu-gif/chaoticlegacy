...
function parseAbilityEffects(text) {
  ...
  working = working.replace(removeInvisibilityRegex, (_, targetWord) => {
    effects.push({
      ...
      target: /opposing/i.test(String(targetWord || "")) ? "opponent" : "self",
      sourceText: _.trim(),
    });
    return " ";
  });

  const infectRegex = /Infect\s+target\s+(Uninfected\s+)?Creature/gi;
  working = working.replace(infectRegex, (_, uninfectedWord) => {
    effects.push({
      ...
      targetSpec: {
        ...
        required: true,
        scope: "all",
        requireUninfected: Boolean(String(uninfectedWord || "").trim()),
      },
      sourceText: _.trim(),
    });
    return " ";
  ...
  const directDamageTargetCreatureRegex = /Deal\s+(\d+)\s+damage\s+to\s+target\s+Creature/gi;
  working = working.replace(directDamageTargetCreatureRegex, (_, amount) => {
    effects.push({
      kind: "dealDamage",
      amount: Number(amount),
      sourceText: _.trim(),
  ...
  const hymnOfElementsRegex =
    /Target Creature gains an Elemental Type of your choice/gi;
  working = working.replace(hymnOfElementsRegex, (_) => {
    effects.push({
