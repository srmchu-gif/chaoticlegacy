...
const CARD_CATALOG_STORAGE = "sql_catalog_v2_typed";
const EFFECT_PARSER_SCHEMA_VERSION = "2026-05-10-dop-hotfix-19-effects-v1";
...
function buildCatalogSourceFingerprint(rootDir) {
  ...
  hash.update(EFFECT_PARSER_SCHEMA_VERSION);
  const sourceFiles = [];
  CARD_JSON_FILENAMES.forEach((fileName) => {
...
const CARD_TYPES = Object.freeze(["creatures", "attacks", "battlegear", "locations", "mugic"]);
const CATALOG_TABLE_BY_TYPE = Object.freeze({
  creatures: "card_catalog_creatures",
  attacks: "card_catalog_attacks",
  battlegear: "card_catalog_battlegear",
  locations: "card_catalog_locations",
  mugic: "card_catalog_mugic",
});
const PAYLOAD_TABLE_BY_TYPE = Object.freeze({
  creatures: "card_payloads_creatures",
  attacks: "card_payloads_attacks",
  battlegear: "card_payloads_battlegear",
  locations: "card_payloads_locations",
  mugic: "card_payloads_mugic",
});

function createCatalogTables(db) {
  CARD_TYPES.forEach((type) => {
    const catalogTable = CATALOG_TABLE_BY_TYPE[type];
    db.exec(`
    ...
    `);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_${catalogTable}_set_rarity ON ${catalogTable}(set_name, rarity);`);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_${catalogTable}_normalized_name ON ${catalogTable}(normalized_name);`);
  });

  db.exec(`
    CREATE TABLE IF NOT EXISTS card_payloads_creatures (
      id TEXT NOT NULL PRIMARY KEY,
      energy REAL NOT NULL DEFAULT 0,
      courage REAL NOT NULL DEFAULT 0,
      power REAL NOT NULL DEFAULT 0,
      wisdom REAL NOT NULL DEFAULT 0,
      speed REAL NOT NULL DEFAULT 0,
      mugicability REAL NOT NULL DEFAULT 0,
      bp REAL NOT NULL DEFAULT 0,
      base REAL NOT NULL DEFAULT 0,
      cost REAL NOT NULL DEFAULT 0,
      fire REAL NOT NULL DEFAULT 0,
      air REAL NOT NULL DEFAULT 0,
      earth REAL NOT NULL DEFAULT 0,
      water REAL NOT NULL DEFAULT 0,
      fire_attack REAL NOT NULL DEFAULT 0,
      air_attack REAL NOT NULL DEFAULT 0,
      earth_attack REAL NOT NULL DEFAULT 0,
      water_attack REAL NOT NULL DEFAULT 0,
  ...
  ["attacks", "battlegear", "locations", "mugic"].forEach((type) => {
    const payloadTable = PAYLOAD_TABLE_BY_TYPE[type];
    db.exec(`
      CREATE TABLE IF NOT EXISTS ${payloadTable} (
        id TEXT NOT NULL PRIMARY KEY,
...
function hasCatalogRows(db) {
  const total = CARD_TYPES.reduce((sum, type) => {
    const tableName = CATALOG_TABLE_BY_TYPE[type];
...
function importCatalogToSql(db, rootDir, options = {}) {
  ...
  const insertCatalogByType = {};
  CARD_TYPES.forEach((type) => {
    insertCatalogByType[type] = db.prepare(`
      INSERT INTO ${CATALOG_TABLE_BY_TYPE[type]} (
        id, name, normalized_name, set_name, rarity, tribe, ability, flavor_text, id_in_set,
        unique_flag, loyal_flag, legendary_flag, image, source_file, raw_json
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
  });
  const insertCreaturePayload = db.prepare(`
    INSERT INTO card_payloads_creatures (
      id, energy, courage, power, wisdom, speed, mugicability, bp, base, cost,
      fire, air, earth, water, fire_attack, air_attack, earth_attack, water_attack, initiative,
      parsed_effects_json, creature_types_raw, creature_types_json, creature_type_keywords_json,
      creature_types_inferred, creature_types_source, image_variants_json
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const insertPayloadByType = {
    attacks: db.prepare(`INSERT INTO card_payloads_attacks (id, stats_json, parsed_effects_json, image_variants_json) VALUES (?, ?, ?, ?)`),
    battlegear: db.prepare(`INSERT INTO card_payloads_battlegear (id, stats_json, parsed_effects_json, image_variants_json) VALUES (?, ?, ?, ?)`),
    locations: db.prepare(`INSERT INTO card_payloads_locations (id, stats_json, parsed_effects_json, image_variants_json) VALUES (?, ?, ?, ?)`),
    mugic: db.prepare(`INSERT INTO card_payloads_mugic (id, stats_json, parsed_effects_json, image_variants_json) VALUES (?, ?, ?, ?)`),
  };
  ...
  try {
    Object.values(PAYLOAD_TABLE_BY_TYPE).forEach((tableName) => db.exec(`DELETE FROM ${tableName};`));
    Object.values(CATALOG_TABLE_BY_TYPE).forEach((tableName) => db.exec(`DELETE FROM ${tableName};`));
    ...
    cards.forEach((card) => {
      ...
      if (type === "creatures") {
        ...
        insertCreaturePayload.run(
          card.id,
          toNumber(stats.energy, 0),
          toNumber(stats.courage, 0),
          toNumber(stats.power, 0),
          toNumber(stats.wisdom, 0),
          toNumber(stats.speed, 0),
          toNumber(stats.mugicability, 0),
          toNumber(stats.bp, 0),
          toNumber(stats.base, 0),
          toNumber(stats.cost, 0),
          toNumber(stats.fire, 0),
          toNumber(stats.air, 0),
          toNumber(stats.earth, 0),
          toNumber(stats.water, 0),
          toNumber(stats.fireAttack, 0),
          toNumber(stats.airAttack, 0),
          toNumber(stats.earthAttack, 0),
          toNumber(stats.waterAttack, 0),
          String(stats.initiative || ""),
...
function buildLibraryFromSql(db, rootDir) {
  ...
  const loadCardsByType = (type) => {
    ...
    if (type === "creatures") {
      rows = db.prepare(`
        SELECT
          c.id, c.name, c.normalized_name, c.set_name, c.rarity, c.tribe, c.ability,
          c.flavor_text, c.id_in_set, c.unique_flag, c.loyal_flag, c.legendary_flag, c.image,
          c.source_file, c.raw_json,
          p.energy, p.courage, p.power, p.wisdom, p.speed, p.mugicability, p.bp, p.base, p.cost,
          p.fire, p.air, p.earth, p.water, p.fire_attack, p.air_attack, p.earth_attack, p.water_attack,
          p.initiative, p.parsed_effects_json, p.creature_types_raw, p.creature_types_json,
          p.creature_type_keywords_json, p.creature_types_inferred, p.creature_types_source, p.image_variants_json
        FROM ${catalogTable} c
        JOIN ${payloadTable} p ON p.id = c.id
        ORDER BY c.name ASC
      `).all();
    } else {
      rows = db.prepare(`
        SELECT
          ...
          c.flavor_text, c.id_in_set, c.unique_flag, c.loyal_flag, c.legendary_flag, c.image,
          c.source_file, c.raw_json,
          p.stats_json, p.parsed_effects_json, p.image_variants_json
        FROM ${catalogTable} c
        JOIN ${payloadTable} p ON p.id = c.id
        ORDER BY c.name ASC
      `).all();
    ...
    rows.forEach((row) => {
      ...
      if (type === "creatures") {
        cardsByType[type].push({
          ...baseCard,
          stats: {
            energy: toNumber(row.energy, 0),
            courage: toNumber(row.courage, 0),
            power: toNumber(row.power, 0),
            wisdom: toNumber(row.wisdom, 0),
            speed: toNumber(row.speed, 0),
            mugicability: toNumber(row.mugicability, 0),
            bp: toNumber(row.bp, 0),
            base: toNumber(row.base, 0),
            cost: toNumber(row.cost, 0),
            fire: toNumber(row.fire, 0),
            air: toNumber(row.air, 0),
            earth: toNumber(row.earth, 0),
            water: toNumber(row.water, 0),
            fireAttack: toNumber(row.fire_attack, 0),
            airAttack: toNumber(row.air_attack, 0),
            earthAttack: toNumber(row.earth_attack, 0),
            waterAttack: toNumber(row.water_attack, 0),
  ...
  CARD_TYPES.forEach((type) => loadCardsByType(type));
